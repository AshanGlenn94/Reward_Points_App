...
function calculatePoints(inputData) {
  ...
  const pointsPerTransaction = inputData.map((transaction) => {
    ...
    if (greaterThan100 > 0) {
      // As per the business logic a customer receives 2 points for every dollar spent greater than $100 in each transaction
      points += greaterThan100 * 2;
    }
    if (transaction.amt > 50) {
      // As per the business logic a customer receives 1 point for every dollar spent greater than $50 in each transaction
      points += 50;
    }
